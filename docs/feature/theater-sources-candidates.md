...
2. **[調査中]** B（RSS候補）・C（HTML一覧）・E（PR TIMES企業別RSS）からレイヤー1データソースを
   再選定する。いずれも仕様書7.「HTMLスクレイピングは必要最小限にする」「利用規約を確認する」
   との整合性を個別に確認する必要がある
3. PR TIMES企業別RSS（E.節）は東宝のみcompany_id確認済み。東映・松竹・ワーナー・ディズニー・
   ギャガ・KADOKAWA等のcompany_id調査が次の調査対象
4. 確認が取れたソースを「劇場情報源」シート（`ID / 名称 / URL / 取得方式 / レイヤー / 有効/無効 /
