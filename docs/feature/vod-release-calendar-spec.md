...
```text
[VOD情報源シート] ──┐
                    ├─→ [discover_vod.py: AI Web検索（Claude/OpenAI併用）]
[AI Web検索既定]  ──┘          ↓
...
- `取得方式` は初期MVPでは `ai`（AI Web検索）のみ実装。`rss` / `html` は行を登録できるが
  規約確認済みのソースが現れるまで実装保留

### 7.2 規約上の制約（theater施策からの継承事項）

KatsumascoreはGoogle AdSenseで収益化しているため、以下は**使用不可・保留**
（詳細は [theater-sources-candidates.md](theater-sources-candidates.md)）。

| ソース | 判断 |
|---|---|
| TMDb API（`/discover` + watch providers） | 商用利用ライセンス（$149/月〜）未契約のため使用不可 |
| PR TIMES企業別RSS | 一般規約6条④「有償目的で企業コンテンツを利用する行為」に抵触リスク |
| 映画.com等ニュースメディアRSS | 複製・転載・公衆送信の明示的禁止 |
| VODサービス公式サイト（Netflixのcoming-soonページ等）の直接スクレイピング | 各サービスの利用規約を個別確認するまで保留。確認できたものだけシートに登録する |

### 7.3 初期MVPの取得方式: AI Web検索
...
```text
ID / 名称 / URL / 取得方式(ai・rss・html) / 対象サービス / 有効/無効 / 規約確認済み / メモ
```

初期データは `取得方式=ai` の1行（AI Web検索）のみ。将来、規約確認済みのRSS等が
見つかったら行を追加するだけで取得対象を増やせる。
