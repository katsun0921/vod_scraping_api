...
`編集部おすすめ`/`編集部コメント`は15.未決定事項#12の運用（管理者が手動選定、
将来自動化を検討）に対応する列。「公式X一覧」の`有効/無効`と同じくSheetsの
チェックボックス（真偽値セル）で管理し、`row.get("編集部おすすめ") is True`の行のみを
`compose_vod.py`が記事冒頭セクション用に抽出する（`_is_active()`と同じ実装パターン）。
承認済み（`投稿状態=承認済み`）の行のみが選定対象になる。
...
- **編集部おすすめ**: 「VOD配信予定」シートの`編集部おすすめ`列（チェックボックス）に
  管理者が手動でチェックした作品を記事冒頭に掲載する。`編集部コメント`列の内容を
  一言コメントとして使い、レビュー記事があればリンクを添える（AIによる自動選定ではなく
  編集者が選ぶことに価値を置く。将来的にAI提案・ルールベース選定への移行も検討、15.未決定事項#12）
...
- [x] 翌週月曜〜日曜の対象期間を計算する
- [x] 公式Xアカウント（取得方式=x）のポストを取得する（生テキストのまま）
- [x] AI Web検索（Claude/OpenAI併用）で対象週の配信開始作品を発見する
...
| 12 | ~~「編集部おすすめ」の選定プロセス~~（運用方針確定） | **管理者が手動で選定する運用とする（将来的にAI提案・ルールベース選定による自動化を検討）**。実装としては「VOD配信予定」シートに`編集部おすすめ`列（チェックボックス + コメント欄）を追加し、承認済み行の中から管理者がチェックを入れる。`compose_vod.py`はチェック済みの行を記事冒頭セクションとして扱う（8./11.2参照） |
