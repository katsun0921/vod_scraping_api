...
2026-08-16〜2026-09-01の間、機械検証つきの自動マージ（`routine-data-auto-merge.yml`）を
試したが撤去した。**`GITHUB_TOKEN`が起こしたpushは他のworkflowを起動しない**ため、
マージAPIを`GITHUB_TOKEN`で叩くと`Routine Import`のpushトリガーが発火せず、
シート追記もSlack通知も走らないまま「マージ成功」で終わっていた。

この期間のルーティンPR（#68・#69）はマージ後に一度も取り込まれていない。

自動マージを再検討する場合は、この制約を回避する手段（マージ後に
`workflow_dispatch`/`repository_dispatch`で明示的に起動する、またはPATでマージする）を
セットで用意する必要がある。
...
いずれも publish より前に走らせ、PRレビューとシート承認の時間を確保するのが狙い。

- VOD: 水曜収集 → `vod_publish` は月曜 07:00 JST（猶予 約4日22時間）
- 劇場: 金曜収集 → `theater_publish` は同じ週の月曜 07:00 JST（猶予 約2日22時間）

**劇場ルーティンは「翌週の金曜」を起点に集める。** 当日の金曜を起点にすると、その週の
`theater_publish` は前日の木曜に終わっているため、集めた行が永久に記事へ載らない。

**取り込み側（`*_import`）の対象週は、実行日ではなく成果物JSONの日付から決める**
（`manual_week.routine_range()`）。ルーティンが推奨曜日以外に走ったり、PRのマージが
別の週にずれ込んだりすると、実行日から逆算した週と成果物の週が食い違い、集めた行が
すべて期間外で捨てられる。実際に #76・#83 は木曜マージだったため
`next_week_range()` が成果物より1週手前を返し、7件ずつがシートにもSlackにも
届かないまま Actions は成功扱いになった。成果物の日付を正とすればこのズレは起きない。
