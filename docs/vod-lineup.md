...
| 実行頻度 | **月次のみ**（毎月 1 日 1 回）。週次実行は行わない |
| 初期対象 | **映画カテゴリ（洋画・邦画）**。洋画＝`lang=en` / 邦画＝`lang=ja` として保持。将来 **アニメ・ドラマ**へ拡張する |
...
初期フェーズは**映画カテゴリ（洋画・邦画）をすべて**対象とする（言語での絞り込みはしない）。

1. **入口で絞る**: 各サービスの「映画」カテゴリ（洋画・邦画）見出し配下から収集する（最も確実）
2. **言語タグ**: 洋画＝`lang="en"`、邦画＝`lang="ja"` をメタデータとして付与する
   （フロント側でのフィルタやバッジ表示に利用可能。出力からは除外しない）
3. ドラマ・アニメ等の非映画カテゴリは対象外（content_type は `movie` 固定）

将来のアニメ・ドラマ拡張は `content_type` と収集対象カテゴリを増やすだけで対応できる設計にする。
