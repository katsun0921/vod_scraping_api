...
⑦ 新規配信を Slack 通知
   ...
   └─ 週次パッチ（weekly_patch.py）はバッチ完了時に新着一覧をまとめて1通で通知
      （詳細: docs/archive/weekly-patch-notifications.md）
...
- 通知フォーマットの詳細は [docs/archive/weekly-patch-notifications.md](./archive/weekly-patch-notifications.md) を参照
...
- `scraping_cooldown_until` で古い作品・長期未配信作品のチェック間隔を動的延長
  ...
  - 詳細仕様: [archive/scraping-frequency.md](archive/scraping-frequency.md)
