...
- **人間の承認が必須**：AIの検索結果は誤り得るため、保存時は投稿状態=`承認待ち`。人間がシートを確認・修正・承認するまで下流処理（週次サマリー・Slack/WP投稿、いずれも未実装）には流れない。
...
- **現時点でのスコープ**：AI発見→期間フィルタ→重複チェック→承認待ちとして保存まで。Katsumascore照合・SNS優先度判定・投稿案生成・Slack通知・Googleカレンダー同期は未実装。
