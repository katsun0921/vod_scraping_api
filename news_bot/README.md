...
- **レイヤー1データソース（再選定中）**：配給会社公式サイト（東宝・東映等）はRSSが無く利用規約で複製・転載を禁止しているため除外。**TMDb APIは一時採用したが撤回した**（KatsumascoreがGoogle AdSenseを掲載しており、TMDbのPersonal Use申請（非商用・無収益の誓約）が事実に反するため）。`fetch_theater._fetch_tmdb()`の実装はコードとして残っているが、商用ライセンス取得までは「劇場情報源」シートに登録しないこと。現在RSS/HTML一覧/PR TIMES企業別RSSから代替候補を再調査中（[theater-sources-candidates.md](../docs/feature/theater-sources-candidates.md)）。
