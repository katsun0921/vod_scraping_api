...
- **取得元管理**：レイヤー1データソース（仕様書7.）が未確定のため、コード変更なしに追加できるよう「劇場情報源」シート（`ID / 名称 / URL / 取得方式 / レイヤー / 有効/無効 / 規約確認済み / メモ`）に取得元を登録する方式にした。RSS一覧と同じ考え方で、シートに行を追加するだけで次回実行から取得対象になる。現状`取得方式`は`rss`のみ対応（html/apiは未実装）。
- **対象期間**：`theater_calendar.week_range()`が実行日から「直近の金曜日〜その翌週木曜日」を計算する（仕様書6.）。
- **公開日抽出**：RSSに構造化フィールドが無い前提のため、タイトル・概要から正規表現でベストエフォート抽出する（`fetch_theater.extract_release_date()`）。抽出できなかった記事は保存せずスキップする（未実装事項、TODO参照）。
- **重複判定**：「公開日 + 正規化タイトル」（`theater_calendar.normalize_title()` / `dedupe_key()`）の完全一致のみ（仕様書9.）。
- **現時点でのスコープ**：取得→期間フィルタ→重複チェック→保存（投稿状態は常に`未判定`）まで。Katsumascore照合・SNS優先度判定・投稿案生成・Slack通知は未実装（新規のGitHub Secretsは不要、既存の`GOOGLE_SHEETS_*`のみ使用）。
