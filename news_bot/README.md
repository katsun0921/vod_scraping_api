...
cronが有効化されており、`news-bot.yml`は2時間おき（仕様書 3.: 1〜2時間おき推奨）、`news-bot-x.yml`は地域ごとに1日1回（日本 1:00 UTC / アメリカ 13:00 UTC）で自動実行される。Actionsタブからの手動実行（`workflow_dispatch`）も引き続き可能。
