...
- **レイヤー1データソース**：配給会社公式サイト（東宝・東映等）はRSSが無く利用規約で複製・転載を禁止しているため除外し、**TMDb API**（`/discover/movie`, region=JP, with_release_type=劇場公開）を採用した（調査詳細は[theater-sources-candidates.md](../docs/feature/theater-sources-candidates.md)）。TMDbの商用ライセンス要否の判定は保留したまま無償利用前提で進めている点に注意（同ドキュメントの「採用判断の記録」参照）。利用するには`TMDB_API_KEY`をGitHub Secretsに登録し、「劇場情報源」シートに`取得方式=tmdb`の行を追加する必要がある。
- **対象期間**：`theater_calendar.week_range()`が実行日から「直近の金曜日〜その翌週木曜日」を計算する（仕様書6.）。TMDb取得時はこの期間をAPIクエリに直接渡す。
- **公開日抽出**：TMDb取得分は構造化データからそのまま取得できる。RSS取得分のみ、タイトル・概要から正規表現でベストエフォート抽出する（`fetch_theater.extract_release_date()`）。抽出できなかった記事は保存せずスキップする。
