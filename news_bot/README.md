...
- **レイヤー1データソース（PR TIMES企業別RSSが最有力候補）**：配給会社公式サイト（東宝・東映等）はRSSが無く利用規約で複製・転載を禁止しているため除外。映画.comRSSも利用規約で複製・転載を明示的に禁止しており除外。**TMDb APIは一時採用したが撤回した**（KatsumascoreがGoogle AdSenseを掲載しており、TMDbのPersonal Use申請（非商用・無収益の誓約）が事実に反するため）。`fetch_theater._fetch_tmdb()`の実装はコードとして残っているが、商用ライセンス取得までは「劇場情報源」シートに登録しないこと。代わりに配給会社が自ら配信するPR TIMES企業別RSS（`https://prtimes.jp/companyrdf.php?company_id={ID}`、東宝/東映/松竹/ワーナー/ディズニー/ギャガ/KADOKAWAのcompany_id確認済み）を最有力候補として選定した。標準RSS形式のため既存の`_fetch_rss()`をそのまま流用できる。正式採用は人間によるPR TIMES利用規約の確認待ち（詳細は[theater-sources-candidates.md](../docs/feature/theater-sources-candidates.md) E.節）。
