...
入力される複数のニュース記事（タイトル・概要）を読み、記事ごとに以下の基準でランク付けしてください。
...
- D: 未確認情報・噂・重複記事など、除外すべきもの（同じ入力内に同一ニュースを報じる記事が複数あれば、1件を除きD判定にする）
...
# 入力フォーマット
以下のようなJSON配列で複数記事が渡されます（indexは0始まり）。
[{"index": 0, "title": "...", "summary": "...", "媒体": "..."}, {"index": 1, ...}, ...]

# 出力フォーマット
必ず入力と同じ件数・同じ順序のJSON配列のみで出力してください。前後に説明文を付けないこと。
[{"index": 0, "rank": "S|A|B|D", "reason": "判定理由を1文で", "confidence": 0.0から1.0の数値}, {"index": 1, ...}, ...]
